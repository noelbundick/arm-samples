...
A scalable set of virtual machines opens up the door for more interesting higher level scenarios. Azure Container Service and Service Fabric are just a couple that take advantage of this. I'm going to play with Service Fabric just because I've touched it this week

> Azure Container Servcie setup is a breeze with the CLI. You can also generate your own ARM templates via [acs-engine](https://github.com/Azure/acs-engine). Looking at the templates generated by acs-engine is another great way to learn more about ARM Templates

I snagged this template from the Azure Portal, so it's not quite as hand crafted as my previous samples. But there's still plenty to learn here
...
* Service Fabric cluster - there's a mountain of options here, more than I can possibly keep up with. I hit the docs often for Service Fabric templates
